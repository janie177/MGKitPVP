...
public class SpellUtil {
	...
	public static void castBloodSpell(Player caster, Entity targetedEntity, boolean ultimate)
	{
		...
		new BloodSpell(140, Effect.WITCH_MAGIC, caster.getLocation().add(0, 1.3F, 0), ultimate ? 10 : 7, targetedEntity.getLocation(), targetedEntity, true, true, Effect.DRAGON_BREATH, caster.getUniqueId().toString());
	}

	public static void castFrostSpell(Player caster, Entity targetedEntity, boolean ultimate)
	{
		EffectUtil.playSound(caster.getLocation(), Sound.ENTITY_ENDERMEN_TELEPORT);
		new FrostSpell(140, Effect.SNOWBALL_BREAK, caster.getLocation().add(0, 1.3F, 0), ultimate ? 10 : 7, targetedEntity.getLocation(), targetedEntity, true, true, Effect.SNOWBALL_BREAK, caster.getUniqueId().toString());
	}

	public static void castFireSpell(Player caster, Entity targetedEntity, boolean ultimate)
	{
		EffectUtil.playSound(caster.getLocation(), Sound.ENTITY_ENDERMEN_TELEPORT);
		new FireSpell(140, Effect.LAVADRIP, caster.getLocation().add(0, 1.3F, 0), ultimate ? 10 : 7, targetedEntity.getLocation(), targetedEntity, true, true, Effect.MOBSPAWNER_FLAMES, caster.getUniqueId().toString());
	}
