...
public class Dwarf implements IHero {
	@Override
	public void doUltimate(Player player) {
		PotionUtil.updatePotion(player, PotionEffectType.INCREASE_DAMAGE, 1, ultimateDuration());
	}
	...
	private static String[] desc = new String[]{"A sturdy dwarf.!", "Has a pickaxe as weapon.", "Gains extra strength", "when killing someone."};
	...
	private static HeroInventory inventory = new HeroInventory(){
		{
			...
			//hand1
			addItem(0, new ItemStack(Material.IRON_PICKAXE)
			{
				{
					addUnsafeEnchantment(Enchantment.DAMAGE_ALL, 4);
				}
			});
		}
	};
	...
	public String ultimateBarMessage() {
		return "Your damage is increased!";
	}
}
